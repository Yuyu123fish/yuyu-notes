...
- 实现`Runnable`接口，并实现`run`方法，这里不能抛异常，且没有返回值
	- 如果是直接调用它的`run()`方法，那么是当前线程执行，可以有无数次
...
## 5.`wait()`,`wait(long)`,`sleep(long)`有什么区别？
这三个方法的效果就是让当前线程**暂时放弃CPU的使用权**，进入阻塞状态。
它们的核心区别有三点：
- 所属类不同
...
## 6.可以直接调用Thread类的run方法吗？
可以直接调用，但是`run()`方法是让**调用者线程**去执行，这就不是多线程了，而如果是调用`start()`来让线程处于就绪状态然后执行，是属于多线程的。
...
## 8.关于`interrupt()`打断唤醒
`thread.interrupt()`不是强制停止线程，而是设置**打断标记**。
如果对于处于`sleep/wait/join`状态下的线程施加`interrupt()`，那么会抛出`InterruptedException`异常并且**清除打断标记**。捕获异常后要么 直接结束线程 要么 重新设置打断标记（如果当前方法不负责结束线程，则应该重新补上打断标记）。
对于处于`synchronized(lock)`等待锁而阻塞的线程，只能设置打断标记，不能让其退出阻塞。

## 9.Java线程卡住怎么办？
生产环境**不能打断点**，要靠日志，监控，线程栈，GC，内存和依赖的服务来排查。
- 先看现象和日志
	- 哪个接口变慢？CPU飙高？内存上涨？FullGC频繁？线程池打满？数据库缓存超时？
- 针对不同情况
	- **CPU飙高**：代码死循环，计算逻辑太复杂，大量线程抢锁，GC太频繁。
	- **接口变慢**：线程等锁，线程池满了，数据库查询慢，Redis外部超时。
	- **内存问题**：内存泄漏，缓存太大，FullGC太频繁。
...
### 1.3.**使用**`Semaphore`**来解决**
`Semaphore`一种信号量手段，用于线程间同步：
- `sem.acquire()`：获取信号量，如果信号量不为0则获取并减一，如果信号量为0则阻塞直到信号量大于0。
- `sem.release()`：释放信号量，让此信号量+1。
```java
package study;
